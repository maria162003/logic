                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (lawyerProfile['location'] != null &&
                                  lawyerProfile['location'].toString().isNotEmpty)
                                Row(
                                  children: [
                                    Icon(
                                      Icons.location_on,
                        ...
                        Row(
                          children: [
                            CircleAvatar(
                              ...
                              child: (lawyerProfile['profile_image_url'] == null &&
                                      lawyerProfile['avatar_url'] == null)
                                  ? const Icon(Icons.gavel, color: Colors.white, size: 25)
                                  : null,
                            ),
                            const SizedBox(width: 12),
                            Expanded(
...
class _CaseChatScreenState extends State<_CaseChatScreen> {
  ...
  Map<String, dynamic>? _otherUserProfile; // Perfil del otro usuario (abogado o cliente)
  ...
  Future<void> _loadOtherUserProfile() async {
    try {
      ...
      if (currentUserType == 'client') {
        ...
        // Primero intentar desde assigned_lawyer_id
        otherUserId = widget.caseData['assigned_lawyer_id'] ?? '';
        
        // Si no existe, intentar desde accepted_proposal
        if (otherUserId.isEmpty) {
          otherUserId = widget.caseData['accepted_proposal']?['lawyer_id'] ?? '';
        }
        
        // Si no existe, intentar desde lawyer_id (campo alternativo)
        if (otherUserId.isEmpty) {
          otherUserId = widget.caseData['lawyer_id'] ?? '';
        }
        
        print('DEBUG: Buscando abogado con ID: $otherUserId');
        
        if (otherUserId.isNotEmpty) {
          _otherUserProfile = await SupabaseService.getLawyerProfile(otherUserId);
          print('DEBUG: Perfil de abogado obtenido: ${_otherUserProfile != null ? "✅" : "❌"}');
        } else {
          print('DEBUG: No hay lawyer_id disponible en el caso');
        }
      } else {
        ...
        if (otherUserId.isNotEmpty) {
          ...
          print('DEBUG: Perfil de cliente obtenido: ${_otherUserProfile != null ? "✅" : "❌"}');
        }
      }
    ...
    if (_otherUserProfile != null) {
      ...
      if (currentUserType == 'client') {
        ...
        userName = _otherUserProfile!['user_profiles']?['full_name'] ?? 
                   _otherUserProfile!['full_name'] ?? 'Abogado';
      ...
      userName = currentUserType == 'client' 
          ? 'Abogado (No asignado)' 
          : 'Cliente';
    }
